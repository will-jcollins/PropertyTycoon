...
class PropertyTileTest {
    ...
    void testGetters() {
        ...
        StreetColor color = StreetColor.BROWN;
        int cost = 60;
        ...
        assertEquals(propTile.getGroup(), color);
        assertEquals(propTile.getNoHouses(), 0);
        assertEquals(propTile.getOwnerID(), -1);

        for (int i = 0; i < PropertyTile.MAXNOHOUSES; i++) {
    ...
    void testAddHouse() {
        ...
        int[] rent = {1, 2, 3, 4, 5, 6};
        PropertyTile propTile = new PropertyTile(name, color, cost, rent);
        for (int i = 0; i < PropertyTile.MAXNOHOUSES - 1; i++) {
